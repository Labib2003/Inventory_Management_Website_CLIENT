...
const ReportBug = () => {
  ...
  const handleReportBug = (event) => {
    ...
    if (user) {
      fetch("https://inventory-management-server-f0ho.onrender.com/bugs", {
        method: "POST",
        body: JSON.stringify({
          title: title,
          user: user.email,
      ...
      <div className="flex justify-around">
        ...
        <div className="my-auto">
          <p className="text-xl text-gray-800 font-semibold mb-3">
            Having an issue? Report it and we will fix it ASAP!
          </p>
          <form onSubmit={handleReportBug} className="flex flex-col">
            ...
            <div className="flex justify-between">
              <input
                className="text-white bg-blue-500 hover:bg-blue-400 rounded-md px-3 py-1 mb-3 cursor-pointer"
                type="submit"
                value="Report Issue"
              ></input>
              <Link
                to="/bugs"
                className="text-white bg-blue-500 hover:bg-blue-400 rounded-md px-3 py-1 mb-3"
              >
                Previous Issues
              </Link>
            </div>
